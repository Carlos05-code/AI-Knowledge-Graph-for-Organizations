...
describe('RecommendationsService', () => {
  ...
  const mockEmbedding = { generateEmbedding: jest.fn().mockResolvedValue(Array(1536).fill(0.1)) };
  ...
  it('should get recommendations', async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-1', title: 'Engineer', department: 'Engineering' });
  ...
  it('should recommend experts with similar roles', async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-1', title: 'Frontend Engineer', department: 'Engineering' });
    mockPrisma.user.findMany.mockResolvedValue([
      {
        id: 'user-2', firstName: 'Jane', lastName: 'Doe', title: 'Frontend Lead',
  ...
  it('should recommend documents based on user messages', async () => {
    ...
    mockPrisma.document.findMany.mockResolvedValue([
      { id: 'doc-1', title: 'Payment API Docs', description: 'Guide', fileType: 'md' },
    ]);
    mockPrisma.meetingParticipant.findMany.mockResolvedValue([]);
    mockPrisma.meeting.findMany.mockResolvedValue([]);
    mockNeo4j.executeRaw.mockResolvedValue([]);
  ...
  it('should get personalized feed', async () => {
    mockPrisma.document.findMany.mockResolvedValue([
      { id: 'doc-1', title: 'Recent Doc', createdAt: new Date(), fileType: 'pdf' },
    ]);
    mockPrisma.meeting.findMany.mockResolvedValue([
      { id: 'mtg-1', title: 'Sprint Review', meetingDate: new Date() },
    ]);
    mockPrisma.notification.findMany.mockResolvedValue([
      { id: 'notif-1', title: 'Doc processed', message: 'Ready', isRead: false, createdAt: new Date() },
    ]);
    mockPrisma.knowledgeGap.findMany.mockResolvedValue([
      { id: 'gap-1', title: 'Missing docs', severity: 'HIGH', resolvedAt: null, createdAt: new Date() },
    ]);
