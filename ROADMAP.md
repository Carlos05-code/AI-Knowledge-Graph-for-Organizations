...
| 25 | Production deployment | done | Compose stack completed (16 services: exporters, promtail, healthchecks, resource limits); K8s hardened (non-root, read-only root FS, PDBs, termination grace); HTTP metrics interceptor (http_requests_total + http_request_duration_seconds); alerts.yml rule set; AKG Backend Overview Grafana dashboard; frontend nginx on 8080 for non-root (uid 101); Ingress TLS + automated provisioning pending |
