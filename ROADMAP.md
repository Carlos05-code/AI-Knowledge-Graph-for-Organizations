...
- ✅ Tests: 82 unit + 62 e2e + 28 frontend tests, all passing
...
4. ✅ VIEWER role enforcement — done: `RolesGuard` now denies every non-GET/HEAD/OPTIONS request for VIEWER (`403 VIEWER role is read-only`), attached to the JWT-only write controllers (chat, upload, meetings, notifications, users), role taken from the DB per request; 13 e2e VIEWER cases + `roles.guard.spec` (6) added; WS chat role-gating remains a known gap; 82 unit + 62 e2e green.
